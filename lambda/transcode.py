...
preset_id='1510432481093-rhpjuv' # from http://docs.aws.amazon.com/elastictranscoder/latest/developerguide/system-presets.html
public_read_grant={'Grantee': {'Type': 'Group', 'URI': 'http://acs.amazonaws.com/groups/global/AllUsers'}, 'Permission': 'READ'}
...
def transcode(input_key, wait=True):
    ...
    while wait and status in (None, 'Submitted', 'Progressing'):
        ...
        if new_status == 'Error':
            ...
            sys.exit(1)
        time.sleep(1)
    for obj in (obj for obj in output_objects(input_key, output_key_prefix, output_key_base) if public_read_grant not in obj.Acl().grants):
...
def output_objects(input_key, output_key_prefix, output_key_base):
    return [obj
        ...
        for obj in s3.Bucket(bn).objects.filter(Prefix=output_key_prefix + output_key_base).all()
        if obj.key != input_key]

# movie_bucket = s3.Bucket(movie_bucket_name)
# transcoded = {obj.key for obj in movie_bucket.objects.filter(Prefix='design-elements/').all()}
# print(transcoded)
